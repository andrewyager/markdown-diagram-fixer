    ...
    for i, line in enumerate(lines):
        if line.strip() == "```":
            if in_code_block:
                ...
                if any(char in block_content for char in diagram_chars) or any(
                    char in block_content for char in ascii_art_chars
                ):
        ...
        if isinstance(obj, dict):
            if obj.get("t") == "CodeBlock":
                ...
                if attr and isinstance(content, str):
                    ...
                    diagram_chars = "┌┐└┘├┤┬┴┼─│┄┅┆┇┈┉┊┋"
                    ascii_art_chars = "+|\\-/"
                    if any(char in content for char in diagram_chars) or any(
                        char in content for char in ascii_art_chars
                    ):
                        ...
                        if fixed_content != content:
                            ...
                            if not os.environ.get("DIAGRAM_FIXER_QUIET"):
                                print(
                                    "diagram-fixer: Fixed diagram in code block",
                                    file=sys.stderr,
                                )
