...
class PrecisionDiagramFixer:
    ...
    def fix_diagram(self, lines):
        ...
        while iteration < max_iterations:
            ...
            if self.debug and len(lines) > 29:  # Check if we have row 30
                ...
                if (
                    last_target_width is not None
                    and current_target_width != last_target_width
                ):
                    print(
                        f"📏 WIDTH CHANGE DETECTED: Target width changed from "
                        f"{last_target_width} → {current_target_width} chars"
                    )
                    print("   This suggests a box expansion occurred")
                last_target_width = current_target_width

            # Find the first row with problems (top-to-bottom approach)
            problem_rows = [
                r for r in sorted(row_analysis.keys()) if row_analysis[r]["problems"]
            ]
            ...
            if self.debug and first_problem_row < max_fixed_row:
                print(
                    f"🚨 REGRESSION DETECTED: Found problem in row {first_problem_row}"
                )
                print(f"   We previously fixed rows up to {max_fixed_row}")
                print("   This suggests our last fix created a new problem!")
                print()
            ...
            if self.debug:
                print("Applied fix, rescanning diagram...")
                print()
    ...
    def detect_boxes(self, lines: List[str]) -> List[dict]:
        ...
        self._count_corner_characters(lines)
        ...
        if self.debug:
            ...
            for i, box in enumerate(boxes):
                print(
                    f"  Box {i+1}: top_left=({box['top_left'][0]}, {box['top_left'][1]}), "
                    f"bottom_right=({box['bottom_right'][0]}, {box['bottom_right'][1]})"
                )
            print()
    ...
    def _trace_complete_box(
        self, lines: List[str], start_row: int, start_col: int
    ) -> dict:
        ...
        if bottom_right_row < len(lines):
            ...
            if (
                top_right_col < len(lines[bottom_right_row])
                and lines[bottom_right_row][top_right_col] == "┘"
            ):
                ...
                for offset in range(-tolerance, tolerance + 1):
                    ...
                    if (
                        0 <= check_col < len(lines[bottom_right_row])
                        and lines[bottom_right_row][check_col] == "┘"
                    ):
                        bottom_right_found = True
                        actual_bottom_right_col = check_col
                        if self.debug:
                            print(
                                f"  Found misaligned bottom-right corner: expected col {top_right_col}, "
                                f"found at col {check_col} (tolerance: {tolerance})"
                            )
                        break
        ...
        return {
            ...
            "bottom_right": (
                max_bottom_row,
                max_right_col,
            ),  # Use max extent, not actual detected
    ...
    def _validate_box(self, lines: List[str], box: dict) -> bool:
        ...
        if self.debug:
            print(
                "Box validation failed, searching for nearby misaligned characters..."
            )
        ...
        for row in range(top_row + 1, bottom_row):
            if row < len(lines):
                if (
                    right_col >= len(lines[row])
                    or lines[row][right_col] not in valid_vertical_chars
                ):
                    ...
                    for offset in range(-5, 6):
                        ...
                        if (
                            0 <= check_col < len(lines[row])
                            and lines[row][check_col] in valid_vertical_chars
                        ):
                            missing_chars_found_nearby += 1
                            if self.debug:
                                print(
                                    f"  Found misplaced │ at row {row}: expected col {right_col}, "
                                    f"found at col {check_col}"
                                )
                            break
        ...
        for row in range(top_row + 1, bottom_row):
            if row < len(lines):
                if (
                    left_col >= len(lines[row])
                    or lines[row][left_col] not in valid_vertical_chars
                ):
                    total_missing_chars += 1
                    # Look within 5 columns for the missing │
                    for offset in range(-5, 6):
                        ...
                        if (
                            ...
                            if self.debug:
                                print(
                                    f"  Found misplaced │ at row {row}: expected col {left_col}, "
                                    f"found at col {check_col}"
                                )
                            break
        ...
        if total_missing_chars > 0:
            ...
            if self.debug:
                print(
                    f"  Missing chars found nearby: {missing_chars_found_nearby}/"
                    f"{total_missing_chars} = {nearby_ratio:.1%}"
                )

            if nearby_ratio >= 0.5:  # If we found at least 50% of missing chars nearby
                if self.debug:
                    print("  ✅ Accepting box - missing characters found nearby")
                return True

        if self.debug:
            print("  ❌ Rejecting box - characters not found nearby")
        return False
    ...
    def save_iteration_state(self, lines, iteration):
        ...
        if (
            self.debug and iteration % 10 == 0
        ):  # Only print every 10th iteration to avoid spam
    ...
    def analyze_diagram_problems(self, lines, iteration=0):
        ...
        try:
            ...
            connection_problems = self.analyze_connection_problems(
                lines, connection_lines, iteration
            )

            # 4. Build row-by-row analysis
            row_analysis = self.build_row_analysis(
                lines, boxes, connection_lines, box_problems, connection_problems
            )
    ...
    def detect_boxes_with_matrix(self, lines, iteration=0):
        ...
        if self.cached_boxes is None:
            ...
            if iteration == 52:
                ...
                if self.debug:
                    print(
                        f"🔒 CACHED {len(boxes)} BOXES at iteration {iteration} for stability"
                    )
                    # Debug the BGP Route Exchange box
                    for i, box in enumerate(boxes):
                        if box["top_left"][0] == 31:  # BGP box starts at row 31
                            print(
                                f"   BGP Box {i+1}: ({box['top_left'][0]},{box['top_left'][1]}) "
                                f"to ({box['bottom_right'][0]},{box['bottom_right'][1]})"
                            )
        else:
            ...
            if (
                self.debug and iteration <= 55
            ):  # Show for first few iterations after caching
                print(
                    f"📋 Using cached boxes from iteration 52 ({len(boxes)} total) for iteration {iteration}"
                )
        ...
        for row_idx, line in enumerate(lines):
            for col_idx, char in enumerate(line):
                if char == "│":
                    ...
                    if (
                        col_idx < len(self.solved[row_idx])
                        and not self.solved[row_idx][col_idx]
                    ):
                        # Regular unsolved pipe
                        unsolved_pipes.append((row_idx, col_idx))
                    elif (
                        col_idx < len(self.solved[row_idx])
                        and self.solved[row_idx][col_idx]
                    ):
                        ...
                        for check_row in range(
                            max(0, row_idx - 5), min(len(lines), row_idx + 6)
                        ):
                            if check_row != row_idx and check_row < len(lines):
                                check_line = lines[check_row]
                                for check_col in range(
                                    max(0, col_idx - 2),
                                    min(len(check_line), col_idx + 3),
                                ):
                                    if (
                                        check_line[check_col] == "│"
                                        and check_col < len(self.solved[check_row])
                                        and not self.solved[check_row][check_col]
                                    ):
    ...
    def analyze_box_problems(self, lines, boxes):
        ...
        for i, box in enumerate(boxes):
            ...
            box_level_problem = self.detect_box_level_misalignment(
                lines, box, top_row, bottom_row
            )
            ...
            # Check left borders
            left_problems = self.check_left_border_alignment(
                lines, top_row, bottom_row, left_col
            )
            problems.extend(left_problems)

            # Check right borders
            right_problems = self.check_right_border_alignment(
                lines, top_row, bottom_row, right_col
            )
    ...
    def detect_box_level_misalignment(self, lines, box, top_row, bottom_row):
        ...
        actual_top_left = self.find_corner_in_line(
            top_line, "┌", search_start, search_end
        )
        actual_top_right = self.find_corner_in_line(
            top_line, "┐", search_start, search_end
        )
        actual_bottom_left = self.find_corner_in_line(
            bottom_line, "└", search_start, search_end
        )
        actual_bottom_right = self.find_corner_in_line(
            bottom_line, "┘", search_start, search_end
        )
    ...
    def correct_box_widths(self, lines, boxes):
        ...
        for i, box in enumerate(boxes):
            ...
            if self.debug:
                ...
                print(
                    f"  Original: ({top_row},{left_col}) to ({bottom_row},{right_col}) - "
                    f"width {right_col - left_col + 1}"
                )
            ...
            for row in range(top_row, bottom_row + 1):
                if row < len(lines):
                    ...
                    valid_chars = (
                        {"│", "┐", "┘"} if row in [top_row, bottom_row] else {"│"}
                    )
                    rightmost_border = self.find_nearest_border(
                        line, right_col, valid_chars
                    )
                    if (
                        rightmost_border is not None
                        and rightmost_border > max_right_col
                    ):
                        max_right_col = rightmost_border
                        if self.debug:
                            print(
                                f"    Row {row}: found border at col {rightmost_border} (extends box width)"
                            )
            ...
            if self.debug:
                print(
                    f"  Corrected: ({top_row},{left_col}) to ({bottom_row},{max_right_col}) - "
                    f"width {max_right_col - left_col + 1}"
                )
                print()
    ...
    def check_left_border_alignment(
        self, lines, top_row, bottom_row, expected_left_col
    ):
        ...
        for row in range(top_row, bottom_row + 1):
            if row < len(lines):
                ...
                actual_border_col = self.find_nearest_border(
                    line, expected_left_col, {expected_char}
                )
    ...
    def check_right_border_alignment(
        self, lines, top_row, bottom_row, expected_right_col
    ):
        ...
        for row in range(top_row, bottom_row + 1):
            if row < len(lines):
                ...
                # Use canonical method to find the nearest border
                actual_border_col = self.find_nearest_border(
                    line, expected_right_col, {expected_char}
                )
        ...
        for i, conn in enumerate(connection_lines):
            ...
            for row_idx, row in enumerate(conn["rows"]):
                if row < len(lines):
                    ...
                    actual_col = (
                        conn["columns"][row_idx]
                        if row_idx < len(conn["columns"])
                        else None
                    )
    ...
    def build_row_analysis(
        self, lines, boxes, connection_lines, box_problems, connection_problems
    ):
    ...
    def apply_fix(self, lines, problem):
        ...
        if problem["issue"] == "missing_connection":
            ...
            if self.debug:
                print(
                    f"  Added missing connection {expected_char} at col {expected_col} in row {row}"
                )
            return lines
        ...
        if self.debug:
            ...
            print(
                f"  Injected '{inject_char}' at col {actual_col} to move {issue} "
                f"from {actual_col} toward {expected_col} in row {row}"
            )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if verbose:
            ...
            # Count changes
            changes = sum(
                1 for orig, fixed in zip(original_lines, fixed_lines) if orig != fixed
            )
